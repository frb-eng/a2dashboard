...
- Third view toggle — Diagram. Alongside the rendered dashboard and the raw JSON, the user can now switch to a layered graph that includes every UI node, binding, endpoint, and state slot in the spec, with arrows showing every cross-layer reference: UI → binding (`rows`), binding → binding (`source` / union tag), binding → endpoint, endpoint → state (param resolution), and dotted writes for `textField`, `setStateAndRefresh` actions, and `filter` state refs. The graph is built deterministically on the client by walking the spec — no LLM, no server round-trip — and rendered directly with [Cytoscape](https://js.cytoscape.org/) using the `cytoscape-dagre` layered DAG layout. Each layer (UI / data / endpoint / state) gets its own shape and colour, dotted edges mark state writes and param refs, and the view re-renders instantly whenever the spec changes.
