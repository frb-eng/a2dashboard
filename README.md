...
- **UI:** `table` for data plus the `row` / `column` / `list` layout containers and the `card` / `tabs` grouping containers, rendered with React + MUI. Container vocabulary (`justify` / `align` / `direction` on flex containers, a single `child` on `card`, `tabs: [{ title, child }]` on `tabs`) is borrowed from Google's a2ui basic catalog.
