...
- **Aggregation engine:** three typed binding variants. `rows` passes an endpoint response through unchanged; `filter` (op `containsIgnoreCase`) keeps only rows whose `field` matches `value` — where `value` is either a literal or a `{ stateKey }` ref so a `textField` can drive the predicate; `limit` truncates another binding's rows to at most `count` entries (the "top N" primitive, paired with an endpoint whose response is already usefully ordered — e.g. `github.repoContributors` ⇒ "top N contributors"). Filters and limits chain through `source`, evaluated inside `useRows` so a `button` press re-applies them. No `group` / `agg` / `join` / `time-bucket` yet — each is a future variant, never a code escape hatch.
