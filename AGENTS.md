...
- **Pagination is a catalogued contract, not an inferred behavior.** Paginated endpoints carry a `pagination` block on their catalog entry: which query param holds the page index, which holds the page size, the page size to use when the spec doesn't set one, and the per-binding `maxPages` cap. A `rows` binding against such an endpoint auto-walks the page sequence until a short page is returned (canonical "end-of-stream" signal) or the cap is hit, and concatenates the pages into one row stream. The engine does no name-sniffing — to make an endpoint paginate, add the `pagination` block in the catalog; to opt out, omit it. New pagination styles (cursor, link-header) land as a discriminated variant on this same block, not as a code escape hatch in the engine.
