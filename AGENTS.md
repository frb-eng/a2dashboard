...
- Pagination is the only "data behavior" the MVP supports beyond raw display. Refresh policy is `manual`, `on-mount`, or `when-state-set` — the last gates the fetch until every named state slot is non-empty (the master-detail right panel's "wait for the row click" semantic; until the gate opens the table renders an idle placeholder, not an error). Keep this surface minimal until a real use case demands more.
