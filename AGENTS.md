...
- **UI primitives:** `table` and `barChart` for data, the `row` / `column` / `list` layout containers, the `card` / `tabs` grouping containers, the `text` / `icon` display leaves, and the `textField` / `button` interactive leaves. Tables compose recursively: every column carries either a `field` path or a `cell` UI node, so a custom cell can hold any subtree (e.g. an icon + bound text). Tables also carry an optional `onRowClick: Action` so clicking a row can drive other panels. `barChart` is the first chart primitive — a vertical bar chart bound to a row binding with `categoryField` (x-axis label) and `valueField` (y-axis number) dotted paths into each row, plus an optional `seriesField` (dotted path) that groups rows into multiple side-by-side series — the multi-series mode is what lets one chart compare the same metric across several entities (e.g. contributors of three repos joined via a `union` binding). No line / pie / scatter / KPI primitives yet.
- **Renderer:** React + MUI (the `@mui/material` `Table` family, or `@mui/x-data-grid` if a feature requires it), plus Highcharts via `highcharts-react-official` for `barChart`. Vocabulary mirrors Google's a2ui basic catalog: `justify` / `align` / `direction` on flex containers, single `child` on `card`, `tabs: [{ title, child }]` on `tabs`, `variant` ∈ h1…h5/caption/body on `text`, a curated `name` enum on `icon`, `variant` ∈ shortText/longText/number/obscured on `textField`, and `variant` ∈ default/primary/borderless plus a single declarative `action` on `button`. Row-scoped data inside table cells flows through a React context, so `text` inside a `cell` resolves its `field` against the current row — and a `setStateAndRefresh` action fired from inside a cell (or from a row click) reads its `valueField` from the same row. `textField` writes into a shared dashboard state map keyed by `stateKey`; endpoint params declared as `{ stateKey }` consume those values at fetch time so a `button` press, or a row-click that wrote a slot, rebuilds every URL against the current state. One renderer; no abstraction over alternatives.
...
- **Aggregation engine:** five typed binding variants. `rows` passes an endpoint response through unchanged; `filter` (op `containsIgnoreCase`) keeps only rows whose `field` matches `value` — where `value` is either a literal or a `{ stateKey }` ref so a `textField` can drive the predicate; `sort` reorders another binding's rows by a single `field` in `asc` / `desc` (numeric when both values are numbers, else string-coerced) — for orderings the endpoint can't express server-side (e.g. `github.userRepos` has no "by stars"); `limit` truncates a source binding's rows to at most `count` entries — paired either with an already-ordered endpoint (e.g. `github.repoContributors` ⇒ "top N contributors") or stacked on top of `sort` for the canonical "top N by X"; `union` concatenates rows from several other bindings, stamping each row with a literal `tag` under `tagField` so a downstream `barChart` with `seriesField: tagField` can render one series per source — the "compare X across N entities in a single chart" primitive. Filters, sorts, and limits chain through `source`; `union` fans out across `sources[*].source`. Cycles and dangling references are caught here. No `group` / `agg` / `join` / `time-bucket` yet — each is a future variant, never a code escape hatch.
...
- The LLM-facing spec should advertise exactly eleven UI primitives (`table`, `barChart`, `row`, `column`, `list`, `card`, `tabs`, `text`, `icon`, `textField`, `button`) and exactly these three endpoints. Don't tell the model about primitives that don't exist.
- Bindings are typed variants: `rows` (raw endpoint), `filter` (predicate over another binding), `sort` (reorder another binding by a field), `limit` ("top N" over another binding), and `union` (concatenate rows from several other bindings, stamping each row with a per-source tag for downstream series-splitting). For derived columns, prefer extending the binding union over inlining logic in the renderer. Cycles and dangling source references are caught by the aggregation engine; never bypass that check.
