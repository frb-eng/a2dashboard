...
WHEN TO RETURN dashboard: null
  ...
  - DO NOT refuse "top N by X" or numeric-ordering requests. Client-side \`sort\` (any row field, asc/desc, numeric when both values are numbers) and \`limit\` (cap row count) ARE supported MVP bindings — see the dataEntries section below. The only filter operator is \`containsIgnoreCase\`, but numeric ordering belongs in \`sort\`, not \`filter\`, so the missing operator on \`filter\` is NOT a reason to refuse. Same for sorting/limiting client-side when the endpoint can't do it server-side: produce the dashboard with a \`rows\` → \`sort\` → \`limit\` pipeline and mention any caveat ("client-side sort over the first page of results") in \`reply\`.
